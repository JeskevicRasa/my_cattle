...
def herd_list(request):
    ...
    paginator = Paginator(herds, 5)  # Show 5 herds per page
...
def update_herd(request, herd_id=None):
    ...
    form.fields['cattle'].queryset = Cattle.objects.filter(deleted=False, loss_method__isnull=True)  # Set the cattle queryset for the form field
